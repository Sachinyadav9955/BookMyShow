...
const NavBar = () => {
  ...
  async function varify() {
    console.log(phoneNumber, "hehehehhehehee");

    try {
      console.log(phoneNumber);
      let res = await axios.post("http://localhost:4000/verify-otp", {
        phoneNumber,
        otp,
      });
      console.log("hiii");
      console.log(res.data.token, "helloooo");
  ...
  return (
    <div className="relative overflow-hidden">
      {/*  Login SignUp Page */}
      <div
        className="bg-[#55555591]  w-full rounded fixed z-10"
        style={loginTogle ? { display: "block" } : { display: "none" }}
      >
        <div className="w-[420px] h-[530px] p-5 bg-white fixed left-[35%] translate-y-28 rounded flex flex-col items-center">
          <p
            onClick={() => setLoginTogle(false)}
            className="absolute right-4 top-4 cursor-pointer"
          >
            <RxCross2 className="text-3xl text-[#555]" />
          </p>
          ...
          <div className="flex items-center gap-8 border border-[#777] px-8 py-2 rounded cursor-pointer mt-8 hover:border-transparent hover:bg-[#f0efef]">
            ...
            <p className=" text-[#555555ba] font-bold">Continue With Google</p>
          </div>
          <div className="flex items-center gap-8 border border-[#777] px-10 py-2 rounded cursor-pointer mt-8  hover:border-transparent hover:bg-[#f0efef]">
            <MdOutlineMailOutline className="text-2xl text-[#555]" />{" "}
            <p className=" text-[#555555ba] font-bold">Continue With Email</p>
          </div>
          <div className="flex items-center gap-8 border border-[#777] px-9 py-2 rounded cursor-pointer mt-6 hover:border-transparent hover:bg-[#f0efef]">
            <FaApple className="text-2xl" />{" "}
            <p className=" text-[#555555ba] font-bold">Continue With Apple</p>
          </div>

          <h2 className="my-6 font-bold text-[#666]">OR</h2>

          {/* for continue?\ */}
          <div style={sendOtp ? { display: "none" } : { display: "block" }}>
            <div className="flex items-center gap-10 text-[#555]">
              <FcCallback className="text-xl text-[#555]" />
              <input
                onChange={(e) => setphoneNumber(e.target.value)}
                type="telephone"
                className=" text-[#555555ba] w-[200px] border-0 border-b border-[#555] rounded-none ml-[-20px]"
                placeholder="e:g  +91 8080808080......"
              />
            </div>
            <div className="mt-12">
          ...
          <div style={sendOtp ? { display: "block" } : { display: "none" }}>
            <div className="flex items-center gap-10 text-[#555]">
              <h4>OTP</h4>
              <input
                type="tel"
            ...
            <div className="mt-12">
              <button onClick={varify} className="w-[300px]">
                varify
              </button>
            </div>
          </div>
      ...
      {/* Side memnu area */}
      <div
        className="bg-[#55555591] h-full w-full rounded fixed z-10"
        onClick={() => setMenuTogle(false)}
        style={menuTogle ? { display: "block" } : { display: "none" }}
      >
        <div className="w-[350px] h-full bg-white fixed right-0">
          ...
          <div className="flex items-center gap-4 mt-5 shadow-xl p-2 pb-4">
            <img
              ...
              alt=""
            />
            <p className="text-[14px] font-bold text-[#747eaae1]">
              Unlock spacial offers & great benefits
            </p>
            <p className="border border-red-600 rounded text-xs w-36 p-2 text-[red] font-bold cursor-pointer">
              Login/Register
            </p>
          </div>

          <div className="flex items-center justify-between p-5">
            <p className="flex items-center gap-3 text-[14px]  text-[#444] font-bold">
              <IoIosNotificationsOutline className="text-2xl" /> Notifications
            </p>{" "}
            <span>⟩</span>
          </div>
          ...
          <div className="flex items-center justify-between p-5">
            <div className="flex items-center gap-3 text-[14px]  text-[#333] font-bold">
              <FaShoppingCart className="text-2xl" />
              <div>
                <h2 className="font-bold">Your Orders</h2>
                <p className="text-xs capitalize">
                  View Your Booking & Purcheses
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between p-5">
            <div className="flex items-center  gap-3 text-[14px]  text-[#888]">
              ...
              <div>
                ...
                <p className="text-xs capitalize">
                  View Commanly Asked Quaries and chats
                </p>
              </div>
            </div>{" "}
          ...
          <div className="flex items-center justify-between p-5">
            <div className="flex items-center  gap-3 text-[14px]  text-[#333]">
              ...
              <div>
                ...
                <p className="text-xs capitalize">
                  View Your Rewards & Unlocks New Ones
                </p>
              </div>
            </div>{" "}
            <CiLock className="text-2xl" />
      ...
      <nav className="flex justify-around items-center my-4">
        <div className="flex gap-8 items-center">
          ...
          <div className="flex items-center border border-[#7777776d] px-2 rounded">
            <CiSearch />{" "}
            <input
              className="w-[450px] border-0"
              type="search"
              placeholder="Search for Movies,Evants,Plays,Sports, and Activities"
            />
          </div>
        </div>
        <div className="flex gap-3 items-center">
          <li>
            Bhopal{" "}
            <RiArrowDropDownLine className="inline text-3xl text-[#555]" />
          </li>
          ...
          <h2 className="ml-8" onClick={() => setMenuTogle(true)}>
